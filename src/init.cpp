...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Pawcoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: pawcoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizePawcoinamount=<n> " + _("Keep N Pawcoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Pawcoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Pawcoin is probably already running."), strDataDir));
    ...
    if (fDaemon)
        fprintf(stdout, "Pawcoin server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Pawcoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Pawcoin to complete") << "\n";
    ...
    nAnonymizePawcoinAmount = GetArg("-anonymizePawcoinamount", 0);
    if(nAnonymizePawcoinAmount > 999999) nAnonymizePawcoinAmount = 999999;
    if(nAnonymizePawcoinAmount < 2) nAnonymizePawcoinAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizePawcoinAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize Pawcoin Amount %d\n", nAnonymizePawcoinAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1Pawcoin+1000 == (.1Pawcoin+100)*10
       10Pawcoin+10000 == (1Pawcoin+1000)*10
    */
